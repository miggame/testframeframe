...
export class ViewManager extends BaseClass {
	 ...
	 * @param view 面板
	 */
	public register(key: number, view: IView): void {
		if (view == null) {
			return;
		}
		if (this._views[key]) {
	 ...
	 * @param newView 新面板
	 */
	public destroy(key: number, newView: IView = null): void {
		var oldView: IView = this.getView(key);
		if (oldView) {
			this.unregister(key);
			oldView.destroy();
			oldView = null;
	 ...
	 *
	 */
	public open(key: number, ...param: any[]): IView {
		var view: IView = this.getView(key);
		if (view == null) {
	 ...
	 * @param param
	 */
	public closeView(view: IView, ...param: any[]): void {
	 ...
	 * @returns {any}
	 */
	public getView(key: number): IView {
