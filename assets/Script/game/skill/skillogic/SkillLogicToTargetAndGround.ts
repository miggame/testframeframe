...
enum SKILLDESCINDEX {
    ...
    SKILLDESCINDEX_IMPACT_TO_OBJ,//给周围目标的子效果
}
 ...
 * 对目标释放
 */
export class SkillLogicToTargetAndGround extends SkillLogicBase {

    constructor() {
        super();
        this.id = SKILLLOGICID.SKILLLOGICID_TARGETANDGROUND;
    }
    ...
    public start(e: entity): boolean {
        ...
        let eEnemy = Game.entityManager.getEntityByID(enemy.enemyID);
        if (!eEnemy)
            return false;
        ...
        let totalNum: number = 1;
        let self = this;
        pool.forEach((te) => {
            if (te === e) {
                return;
            }
            if (te.id === enemy.enemyID)
                return;
            ...
            let pos2 = <PostionComponent>eEnemy.getCompnentByType(PostionComponent);
        ...
        for (let i = 0; i < t.enemy.length; ++i) {
            if (t.enemy[i] > 0) {
                ...
                if (i == 0) {
                    ...
                    value = this.getValueOnceValue(t.skillInstanceID, SKILLDESCINDEX.SKILLDESCINDEX_IMPACT_TO_OBJ);
                }
                if (value > 0) {
